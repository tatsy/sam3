...
def download_imgs(args, image_uuids):
    ...
    for uuid in tqdm(image_uuids, desc='Downloading images'):
        ...
        file_name = Path(args.processed_images_folder) / f'{image.uuid}.{image.url.split(".")[-1]}'
...
def main():
    ...
    parser.add_argument('--num-procs', type=int, default=16, help='Number of parallel processes')
    ...
    chunks = [all_uuids[i : i + chunk_size] for i in range(0, len(all_uuids), chunk_size)]
